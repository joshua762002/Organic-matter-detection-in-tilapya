...
<div class="col-md-6"><div class="card shadow"><div class="card-header bg-primary text-white">Organic Matter Distribution</div><div class="card-body text-center"><div class="chart-container"><canvas id="organicChart"></canvas></div></div></div></div>
</div>
...
<table class="table table-hover table-bordered" id="detections-table">
<thead><tr><th>Staff Name</th><th>Pond</th><th>Sample Code</th><th>Organic Level</th><th>Temp</th><th>pH</th><th>Status</th><th>Date</th></tr></thead>
<tbody></tbody>
</table>
</div></div>
...
let organicChart = new Chart(ctx,{type:'doughnut',data:{labels:['Safe','Moderate','High'],datasets:[{data:[0,0,0],backgroundColor:['#2ecc71','#f39c12','#e74c3c']}]},options:{plugins:{legend:{position:'bottom'}},cutout:'65%'}});

// Status color
function statusColor(status){if(status=="Safe") return '#2ecc71';if(status=="Moderate") return '#f39c12';if(status=="High") return '#e74c3c';return '#95a5a6';}
...
function updateDashboard(){
    ...
    ponds.forEach(p=>{
        ...
        // table
        let badge = p.status=="High"?'<span class="badge bg-danger">High</span>':p.status=="Moderate"?'<span class="badge text-dark" style="background:#f39c12;">Moderate</span>':'<span class="badge bg-success">Safe</span>';
        tableHTML += `<tr><td>${p.full_name}</td><td>${p.pond_name}</td><td>${p.sample_code}</td><td>${p.organic_level}</td><td>${p.water_temperature}</td><td>${p.ph_level}</td><td>${badge}</td><td>${p.detected_at}</td></tr>`;
        // map
